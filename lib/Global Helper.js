"use strict";
/*global Node, Platform, state, Tab, Undo */
...
Array.prototype.moveTabs = function (id, index, undo) {
    ...
    if (undo !== false && Options.get("undo.move-tabs")) {
        ...
        if (this.length === 1) {
            state.undoBar.show("You moved " + this.length + " tab.");
        } else {
            state.undoBar.show("You moved " + this.length + " tabs.");
        }
    }
};


//Function.store = function (saved, action) {
//
//    if (typeof action ===
//};
