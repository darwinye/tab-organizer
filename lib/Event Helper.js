"use strict";
/*global chrome, Options, state, Tab, UI, Window */
...
var action = {
    search: (function () {
        ...
        function findCaller(string, keywords) {
            for (var i = 1; i <= string.length; i += 1) {
                var slice = keywords[string.slice(0, i)];
                if (typeof slice === "function") {
                    var match = string.slice(i);

                    return function (array) {
                        var result = slice(array, match);
                        if (result !== null) {
                            return result;
                        } else {
        ...
        var keywords = {
            ...
            "-": function (array, match) {
                ...
                var normal = findCaller(match, keywords)(array);
                return array.filter(function (item) {
            ...
            "is:selected": function (array, match) {
                ...
                return array.filter(function (item) {
                    return item.hasAttribute("data-queued");
                });
            },
            ...
            /*! "bookmark:": function (array, match) {
                var bookmarks = chrome.bookmarks.search(match, function (matches) {
                    console.log(matches);
                });
                return array;
                return array.filter(function (item) {
            ...
            "same:url": function (array, match) {
                ...
                array.forEach(function (item) {
                    var url = nohash.exec(item.tab.url)[1];
                    //console.log(url);
...
//            });
    ...
    returnTitle: function (index) {
        var value = Options.get("titles")[index];
        return (value) ? value : index + 1;
    },
    unselectWindow: function () {
        var query = document.querySelector(".window[data-selected]");
        if (query) {
    ...
    attachEvents: function (element) {
        chrome.windows.onCreated.addListener(function (win) {
            if (win.type === "normal") {
                chrome.tabs.getAllInWindow(win.id, function (array) {
                    //console.warn("windows.onCreated");

                    win.tabs = array;
                    element.insertBefore(Window.create(win), element.lastChild);
        ...
        chrome.tabs.onUpdated.addListener(function (id, info, tab) {
            var list, node = state.tabs[id];

            if (node && (list = node.parentNode)) {
                list.replaceChild(Tab.create(tab), node);
        ...
        chrome.tabs.onSelectionChanged.addListener(function (id, info) {
            ...
            if (list && node) {
                if ((list = list.querySelector("[data-selected]"))) {
        ...
        chrome.windows.onRemoved.addListener(function (id) {
            ...
            Options.get("titles").remove(list.tabIcon.indexText.value);
