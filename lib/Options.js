...
//                    page.console.warn("Not doing anything");
                ...
                events.forEach(function (item) {
                    clone.remove(item.name, item.action, item.type);
                });
            }
            addEventListener("beforeunload", remove, true);
            addEventListener("unload", remove, true);

            clone.on = function (name, action, type) {
                events.push({
                    name: name,
                    action: action,
                    type: type
...
//!                    var self = this;
                ...
                event[name].forEach(function (item) {
//!                        if (!state.shouldStop) {
                    item.apply(info, args);
//!                        item.call(self, info);
//!                        }
//                        try {
//
...
//        Options.event = Object.create(Options.event);
        Options.event = Options.event.decouple();
    } else {
        Options = dictionary();
