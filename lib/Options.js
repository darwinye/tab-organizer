                ...
                if (!contexts[url]) {
                    contexts[url] = {
                        action: (function () {
                            ...
                            Options.window.addEventListener("beforeunload", function () {
                                //Options.root.console.log(window);
                                //Options.root.console.log(url, contexts[url].list);
                                contexts[url].list.forEach(function (item) {
                                    Options.removeEventListener(item.name, item.action, item.type);
                                });
                            }, true);
                            return function (name, action, type) {
                                //Options.root.console.log(location.href);
                                contexts[url].list.push({
                                    name: name,
                                    action: action,
                                    type: type
                                });
